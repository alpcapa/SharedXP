...
CREATE POLICY "host_sports_owner" ON public.host_sports FOR ALL USING (
  auth.uid() = (SELECT user_id FROM public.host_profiles WHERE id = host_profile_id)
);
...
CREATE POLICY "host_sport_images_owner" ON public.host_sport_images FOR ALL USING (
  host_sport_id IN (
    SELECT hs.id FROM public.host_sports hs
    WHERE hs.host_profile_id IN (
