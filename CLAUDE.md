...
`capacitor.config.json` at the repo root configures the Capacitor wrapper (`appId: com.sharedxp.app`, `webDir: dist`). The `@capacitor/ios` and `@capacitor/android` packages are installed; native project folders (`ios/`, `android/`) are generated by `npx cap add ios` / `npx cap add android` and are not committed. To build natively: `npm run build && npx cap sync` then open the platform project in Xcode / Android Studio. The web PWA and the Capacitor native build share the same `dist/` output.
